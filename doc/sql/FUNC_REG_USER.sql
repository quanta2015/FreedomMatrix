-- ----------------------------
-- Procedure structure for FUNC_REG_USER
-- ----------------------------
CREATE DEFINER=`root`@`%` PROCEDURE `FUNC_REG_USER`(IN `data` varchar(21812))
BEGIN
	...
	while `_index` < `json_items` do
		...
		insert into `exp` (pid,proj_name,date_from,date_to,work_role,work_lang,work_proj,work_detl)
	...
	select * from account a, exp e where a.id = e.pid and a.id = pid;
END